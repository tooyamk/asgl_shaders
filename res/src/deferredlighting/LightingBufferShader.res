...
property Buffer vertexBuffer {name = "Vertex and TexCoord (x, y z, w)";}
...
#define LIGHTING_FRAG(lightingType, factor, worldNormal, lightingAtt, worldVertex) {
	...
	factor.w += blinnPhoneSpecularFactor(worldNormal, lightingDir, viewDir, constant(specularAttributes).x);
	...
	float3 viewDir = normalize(VIEW_WPOS.xyz - worldVertex.xyz);
	factor.w = intensity * blinnPhoneSpecularFactor(worldNormal, lightingDir, viewDir, constant(specularAttributes).x);
...
float4 vert (InOut out) {
	float4 data = attribute(vertexBuffer);
	out.value = data;
...
float4 frag (InOut in) {
	float4 color = tex(in.value.zw, sourceTex);
	
	float4 pos = 1;
	pos.xy = in.value.xy;
	pos.z = decodeFloatFromColor2(color.xy);
	pos = m44(pos, M44_P2W);
	pos /= pos.w;
	
	float3 nrm = normalize(decodeNormalFromColor2(color.zw));
	
	float4 factor = 0;
	
	LIGHTING_FRAG(LIGHT0, factor, nrm, lightingFragAtt0, pos)
	//LIGHTING_FRAG(LIGHT1, factor, nrm, lightingFragAtt1, pos)
	//factor.xyz = nrm;
	//factor.w = 1;
	return factor;
}